...
import TopPromoBanner from './TopPromoBanner.tsx';
...
export default function TopNavBar() {
  ...
  return (
    <header className="fixed top-0 w-full z-30 flex flex-col">
      <TopPromoBanner />
      <nav className="flex justify-between items-center px-margin-mobile md:px-margin-desktop h-20 bg-surface border-b border-on-surface">
        <div className="flex items-center gap-8">
          ...
          <div className="hidden md:flex gap-8 items-center">
            <Link
              className={`text-label-caps font-label-caps pb-1 pt-1 border-t-2 border-t-transparent border-b-2 transition-all duration-300 ${isActive('/') ? 'text-primary border-b-primary' : 'text-secondary border-b-transparent hover:text-on-surface'
                }`}
              to="/"
            >
              BLUEPRINTS
            ...
            <Link
              className={`text-label-caps font-label-caps pb-1 pt-1 border-t-2 border-t-transparent border-b-2 transition-all duration-300 ${isActive('/product/seating')
                  ? 'text-primary border-b-primary'
                  : 'text-secondary border-b-transparent hover:text-on-surface'
                }`}
              to="/product/seating"
            ...
            <Link
              className={`text-label-caps font-label-caps pb-1 pt-1 border-t-2 border-t-transparent border-b-2 transition-all duration-300 ${isActive('/product/storage')
                  ? 'text-primary border-b-primary'
                  : 'text-secondary border-b-transparent hover:text-on-surface'
                }`}
              to="/product/storage"
